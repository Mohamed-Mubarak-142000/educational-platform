...
import { getMyUnitStudents, type StudentSubscriptionEntry } from '@/api/teacherApi';
...
const buildMonthLabels = (count: number, locale: string) => {
  ...
  for (let i = count - 1; i >= 0; i -= 1) {
    ...
    labels.push(date.toLocaleString(locale, { month: 'short', year: '2-digit' }));
  }
  return labels;
};
...
export default function TeacherOverview() {
  ...
  const [studentsTab, setStudentsTab] = useState<'list' | 'subscriptions'>('list');
  ...
  const unitStudents: StudentSubscriptionEntry[] = unitStudentsData?.students ?? [];
      ...
      <div className="grid grid-cols-1 xl:grid-cols-2 gap-6">
        ...
        <Card className="border border-slate-200 dark:border-slate-800 bg-white/80 dark:bg-slate-900/70">
          <CardHeader className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
            ...
            <ViewToggle value={contentView} onChange={(v) => setContentView(v as 'line' | 'bar')} opts={lineBarOpts} />
          </CardHeader>
          <CardContent className="h-72">
            {contentView === 'line'
      ...
      <div className="bg-white dark:bg-slate-900 rounded-2xl border border-slate-200 dark:border-slate-800 overflow-hidden">
        <div className="flex items-center justify-between px-6 py-4 border-b border-slate-100 dark:border-slate-800">
          <div className="flex items-center gap-1 rounded-full border border-slate-200 dark:border-slate-700 overflow-hidden">
            {([
              ...
              { key: 'subscriptions', label: t('subscriptions'), icon: CreditCard },
        ...
        {unitStudentsLoading ? (
          <div className="p-6 space-y-3">
            {[1, 2, 3].map((i) => <SkeletonBlock key={i} className="h-14 rounded-xl" />)}
          </div>
          ...
          <div className="divide-y divide-slate-100 dark:divide-slate-800">
            {unitStudents.map((entry) => (
              <div key={entry.student._id} className="flex items-center gap-4 px-6 py-4">
                <div className="flex-shrink-0">
                  {entry.student.profileImage ? (
                    <img src={entry.student.profileImage} alt={entry.student.name} className="w-10 h-10 rounded-full object-cover" />
                  ) : (
                ...
                <div className="flex-1 min-w-0">
                  <p className="text-sm font-medium text-slate-800 dark:text-slate-200 truncate">{entry.student.name}</p>
                  <p className="text-xs text-slate-500 dark:text-slate-400 truncate">{entry.student.email}</p>
                </div>
            ...
            <table className="w-full text-sm">
              <thead>
                <tr className="bg-slate-50 dark:bg-slate-800/50 text-xs font-semibold text-slate-500 dark:text-slate-400 uppercase tracking-wider">
                  ...
                  <th className="px-4 py-3 text-left">{t('subscriptionStatus')}</th>
                  <th className="px-4 py-3 text-left">{t('enrolledUnits')}</th>
                </tr>
              </thead>
              <tbody className="divide-y divide-slate-100 dark:divide-slate-800">
                {unitStudents.map((entry) => (
                  <tr key={entry.student._id} className="hover:bg-slate-50 dark:hover:bg-slate-800/40 transition-colors">
                    <td className="px-6 py-3 font-medium text-slate-800 dark:text-slate-200">{entry.student.name}</td>
                    <td className="px-4 py-3 text-slate-500 dark:text-slate-400 capitalize">{entry.subscription?.plan ?? '—'}</td>
                    <td className="px-4 py-3"><SubscriptionBadge status={entry.subscription?.status} /></td>
                    <td className="px-4 py-3 text-slate-500 dark:text-slate-400">{entry.enrolledUnits.length}</td>
                  </tr>
                ))}
              </tbody>
            </table>
