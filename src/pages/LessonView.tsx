...
import { getAssignmentContent, type AssignmentContentUnit } from '@/api/teacherAssignmentApi';
import { getMySubscriptionRequests } from '@/api/subscriptionApi';
...
function SkeletonLine({ className }: { className: string }) {
  return <div className={`rounded bg-slate-200 dark:bg-slate-800 animate-pulse ${className}`} />;
}
...
function StepProgress({ active, completed }: { active: StepKey; completed?: boolean }) {
  ...
  return (
    <div className="flex flex-wrap items-center gap-2">
      {STEP_ORDER.map((step, idx) => {
        ...
        const isDone = completed || STEP_ORDER.indexOf(step) < STEP_ORDER.indexOf(active);
        return (
            ...
            <div className="pb-1">
              {lessons.map((lesson, idx) => (
                <SidebarLesson
                  ...
                  isStartHere={!!isFirstUnit && (lesson.order === 1 || idx === 0)}
                />
              ))}
            </div>
          </motion.div>
...
function SidebarLesson({
  ...
  return (
    <div className="space-y-1">
      ...
      >
        {locked ? (
          <Lock className={`w-3.5 h-3.5 flex-shrink-0 ${isActive ? 'text-blue-600' : 'text-slate-400'}`} />
        ) : lesson.videoUrl ? (
  ...
  if (showSkeleton) {
    return (
      <div className="space-y-4">
        <SkeletonLine className="h-4 w-32" />
        {[1, 2].map((i) => (
          <div key={i} className="border border-slate-200 dark:border-slate-800 rounded-xl overflow-hidden animate-pulse">
                  ...
                  {quizOpen && (
                    <div className="space-y-3">
                      ...
                      <div className="flex justify-end">
                        <Button size="sm" variant="outline" onClick={() => setQuizOpen(false)}>
                          {t('close')}
                        </Button>
                      </div>
                    </div>
              ...
              <div className="flex-1 min-w-0">
                <div className="flex items-center gap-2 mb-1">
                  ...
                  <span className="text-xs text-slate-400 dark:text-slate-500">
                    {comment.createdAt ? new Date(comment.createdAt).toLocaleDateString(locale) : t('notAvailableShort')}
                  </span>
                </div>
...
export default function LessonView() {
  ...
  const { data: assignmentContent, isLoading: assignmentContentLoading } = useQuery({
  ...
  const assignmentTeacherId = typeof assignment?.teacherId === 'string' ? assignment.teacherId : assignment?.teacherId?._id;
  const assignmentGradeId = typeof assignment?.gradeId === 'string' ? assignment.gradeId : assignment?.gradeId?._id;
  const assignmentSubjectId = typeof assignment?.subjectId === 'string' ? assignment.subjectId : assignment?.subjectId?._id;
  ...
  const assignmentLesson = assignmentContent?.units
    .flatMap((unit) => unit.lessons.map((lessonItem) => ({ ...lessonItem, unitId: unit._id })))
    .find((item) => item._id === activeLessonId);

  const activeUnitId = (lesson?.unitId as string | undefined) ?? assignmentLesson?.unitId;
  ...
  const lessonStatus = (lessonError as any)?.response?.status as number | undefined;
  const lessonLocked = fromStudent && (lessonStatus === 403 || !!assignmentLesson?.locked);

  const pendingRequest = myRequests.find((req) => {
    if (req.status !== 'Pending') return false;
    const reqTeacherId = typeof req.teacherId === 'string' ? req.teacherId : req.teacherId?._id;
    const reqSubjectId = typeof req.subjectId === 'string' ? req.subjectId : req.subjectId?._id;
    const reqGradeId = typeof req.gradeId === 'string' ? req.gradeId : req.gradeId?._id;
    const reqUnitId = typeof req.unitId === 'string' ? req.unitId : req.unitId?._id;
    if (assignmentTeacherId && reqTeacherId !== assignmentTeacherId) return false;
    if (assignmentSubjectId && reqSubjectId !== assignmentSubjectId) return false;
    if (assignmentGradeId && reqGradeId !== assignmentGradeId) return false;
    if (req.type === 'unit' && activeUnitId && reqUnitId !== activeUnitId) return false;
    return true;
  });

  const sidebarUnits: SidebarUnitItem[] = fromStudent && assignmentContent
  ...
  const sidebarLoading = fromStudent
    ? assignmentContentLoading
    : unitsLoading;
  ...
  const sortedUnits = [...(sidebarUnits ?? [])].sort((a, b) => (a.order ?? 0) - (b.order ?? 0));
  const lessonSequence: Lesson[] = (() => {
    if (fromStudent && assignmentContent?.units?.length) {
      return sortedUnits.flatMap((unit) =>
        [...(unit.lessons ?? [])].sort((a, b) => (a.order ?? 0) - (b.order ?? 0))
      );
    }
    if (activeUnitLessons.length > 0) {
      return [...activeUnitLessons].sort((a, b) => (a.order ?? 0) - (b.order ?? 0));
    }
    return [];
  })();

  const currentLessonIndex = lessonSequence.findIndex((item) => item._id === activeLessonId);
  const prevLesson = currentLessonIndex > 0 ? lessonSequence[currentLessonIndex - 1] : null;
  const nextLesson = currentLessonIndex >= 0 && currentLessonIndex < lessonSequence.length - 1
  ...
  useEffect(() => {
    ...
    const handleTimeUpdate = () => {
      ...
      progressMutation.mutate({ lessonId: activeLessonId, watchedPercentage: percentage });
    };
  ...
  const handleLessonClick = (selectedLesson: Lesson) => {
    ...
    navigate(`/lesson/${selectedLesson._id}?subjectId=${resolvedSubjectId}${fromParam}${assignmentParam}`, { replace: true });
  };
  ...
  const handlePartClick = (selectedLesson: Lesson, part: LessonPart) => {
    ...
    const fromParam = fromStudent ? '&from=student' : '';
    const assignmentParam = assignmentId ? `&assignmentId=${assignmentId}` : '';
    navigate(`/lesson/${selectedLesson._id}?subjectId=${resolvedSubjectId}${fromParam}${assignmentParam}&partId=${part._id}`, { replace: true });
  };

  const backPath = fromStudent
    ? resolvedSubjectId ? `/student/subjects/${resolvedSubjectId}` : '/student/learn'
    : resolvedSubjectId ? `/admin/subjects/${resolvedSubjectId}` : '/admin/subjects';
      ...
      <main className="flex-1 min-w-0 overflow-y-auto">
        ...
        <div className="sticky top-0 z-20 bg-white dark:bg-slate-950 border-b border-slate-200 dark:border-slate-800">
          ...
          <div className="px-4 sm:px-6 pb-3">
            <StepProgress active={activeStep} completed={completedLessonId === activeLessonId} />
          </div>
        </div>
        ...
        {isLoading ? (
          ...
          >
                ...
                <div className="rounded-2xl border border-slate-200 dark:border-slate-800 bg-white/90 dark:bg-slate-950/90 backdrop-blur px-5 py-4 shadow-lg">
                  <div className="flex items-start gap-4">
                    ...
                    <div className="flex-1">
                      {pendingRequest ? (
                        ...
                        <>
                          ...
                          <p className="text-xs text-slate-500 dark:text-slate-400 mt-1">
                            {t('lessonLockedDesc', { defaultValue: 'Subscribe to unlock this lesson and its quizzes.' })}
                          </p>
                        </>
                      )}
                    </div>
                    <div className="flex items-center gap-2">
                      ...
                      {!pendingRequest && assignmentTeacherId && resolvedSubjectId && (
                        <Button
                          size="sm"
                          className="bg-blue-600 hover:bg-blue-700 text-white"
                          onClick={() =>
                            navigate(`/student/subjects/${resolvedSubjectId}/teachers/${assignmentTeacherId}`, {
                              state: { gradeId: assignmentGradeId, assignmentId },
                            })
                          }
                        >
                          {t('subscribeCta')}
            ...
            <div className={lessonLocked ? 'pointer-events-none select-none opacity-70' : ''}>
            {/* Step 1: Learn */}
            <StepSection
              step={1}
            ...
            >
              ...
              <RenderIfExists value={lesson.videoUrl} emptyMessage={t('lessonNoVideo')} emptyClassName="py-10">
                <div className="rounded-2xl overflow-hidden bg-black shadow-2xl aspect-video w-full">
                  {isYouTubeUrl(lesson.videoUrl) ? (
                    ...
                    <video ref={videoRef} src={lesson.videoUrl} controls className="w-full h-full" />
                  )}
                </div>
              </RenderIfExists>
              ...
              <div className="grid grid-cols-1 lg:grid-cols-2 gap-5">
                <div className="bg-slate-50 dark:bg-slate-900/60 rounded-xl p-5 border border-slate-200 dark:border-slate-800">
                  ...
                  <RenderIfExists value={lesson.description} emptyMessage={t('lessonNoDescription')} emptyClassName="py-4">
                    <p className="text-slate-600 dark:text-slate-400 leading-relaxed whitespace-pre-wrap">
                      {lesson.description}
                    </p>
                  </RenderIfExists>
                  <RenderIfExists value={lesson.imageUrl} emptyMessage={t('noImage')} emptyClassName="py-4">
                ...
                <div className="bg-slate-50 dark:bg-slate-900/60 rounded-xl p-5 border border-slate-200 dark:border-slate-800 space-y-4">
                  ...
                  <RenderIfExists value={lesson.pdfUrl} emptyMessage={t('lessonNoPdf')} emptyClassName="py-4">
                    <div className="space-y-3">
                  ...
                  <div className="pt-3 border-t border-slate-200 dark:border-slate-800">
                    ...
                    <RenderIfExists value={lesson.audioUrl} emptyMessage={t('lessonNoAudio')} emptyClassName="py-3">
              ...
              <div className="bg-purple-50 dark:bg-purple-900/10 rounded-2xl p-5 border border-purple-200 dark:border-purple-800/30 space-y-4">
                ...
                <RenderIfExists value={lesson.modelUrl || lesson.modelExplanation} emptyMessage={t('lessonNo3dModel')} emptyClassName="py-6">
                  <div className="space-y-4">
                    <RenderIfExists value={lesson.modelUrl} emptyMessage={t('lessonNo3dModel')} emptyClassName="py-4">
                    ...
                    <RenderIfExists value={lesson.modelExplanation} emptyMessage={t('noData')} emptyClassName="py-4">
              ...
              {lessonQuiz ? (
                <div className="space-y-4">
                  <div className="bg-blue-50 dark:bg-blue-900/10 rounded-2xl p-6 border border-blue-200 dark:border-blue-800/30 flex items-center justify-between gap-4">
                    <div className="flex items-center gap-3 min-w-0">
                      <div className={`w-10 h-10 rounded-xl flex items-center justify-center flex-shrink-0 ${user?.role === 'Teacher' ? 'bg-amber-500' : 'bg-blue-600'}`}>
                        {user?.role === 'Teacher' ? (
                      ...
                      <div className="min-w-0">
                        ...
                        <p className="text-xs text-slate-500 dark:text-slate-400 mt-0.5">
                          {user?.role === 'Teacher' ? t('quizPreviewTitle') : t('lessonQuizSubtitle')}
                        </p>
                      </div>
                    </div>
                    <Button
                      variant={user?.role === 'Teacher' ? 'outline' : 'default'}
                      className={user?.role === 'Teacher' ? 'border-amber-500 text-amber-700 dark:text-amber-400 flex-shrink-0 hover:bg-amber-50 dark:hover:bg-amber-900/20' : 'bg-blue-600 hover:bg-blue-700 text-white flex-shrink-0'}
                      onClick={() => setQuizOpen((prev) => !prev)}
                    >
                      {user?.role === 'Teacher' ? (
                        <><Eye className="w-4 h-4 mr-2" />{t('previewQuiz')}</>
                      ) : (
                        <><ClipboardList className="w-4 h-4 mr-2" />{t('startQuiz')}</>
                      )}
                    </Button>
                  </div>
                ...
                <EmptyState description={t('lessonNoQuiz', { defaultValue: 'No quiz yet for this lesson.' })} className="py-8" />
              )}
            </StepSection>
            ...
            >
              <div className="flex flex-wrap items-center justify-between gap-4">
                <div>
                  <p className="text-sm font-semibold text-slate-800 dark:text-slate-200">{t('lessonCompletionTitle', { defaultValue: 'Mark this lesson complete' })}</p>
                  <p className="text-xs text-slate-500 dark:text-slate-400 mt-1">{t('lessonCompletionDesc', { defaultValue: 'Finish this step to unlock your next lesson.' })}</p>
                </div>
                {fromStudent && (
                  <Button
                    size="sm"
                    ...
                    disabled={progressMutation.isPending || completedLessonId === activeLessonId}
                  >
              ...
              {nextLesson && (
                <div className="flex items-center justify-between gap-4 bg-blue-50 dark:bg-blue-900/10 rounded-xl p-4 border border-blue-200 dark:border-blue-800/30">
                  <div className="min-w-0">
                    <p className="text-xs text-blue-600 dark:text-blue-400 uppercase tracking-wide font-semibold">{t('nextUp', { defaultValue: 'Next up' })}</p>
