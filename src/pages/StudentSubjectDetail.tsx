...
import { uploadPaymentProof } from '@/api/adminApi';
import { type AssignmentContentUnit, type AssignmentContentLesson } from '@/api/teacherAssignmentApi';
import { createSubscriptionRequest } from '@/api/subscriptionApi';
...
function QuizBadge({ attachedToId, label }: { attachedToId: string; label: string }) {
  ...
  return (
    <>
      ...
      <StudentQuizModal open={open} onClose={() => setOpen(false)} quizId={quiz._id} quizTitle={quiz.title || label} timeLimitMinutes={quiz.timeLimit} />
    </>
  );
}
  ...
  return (
    <div className="border rounded-xl overflow-hidden shadow-sm border-slate-200 dark:border-slate-800 animate-pulse">
      ...
      <div className="bg-white dark:bg-slate-950">
        {[1, 2, 3].map((i) => (
          <div key={i} className="flex items-center gap-4 px-6 py-3.5 border-t border-slate-100 dark:border-slate-800/50">
    ...
    <div className={`border rounded-xl overflow-hidden shadow-sm transition-colors ${
      ...
      >
        <div className="flex items-center gap-3 min-w-0">
          <span className={`w-8 h-8 rounded-lg text-white text-sm font-bold flex items-center justify-center flex-shrink-0 ${isUnlocked ? 'bg-blue-600' : 'bg-slate-400 dark:bg-slate-600'}`}>
            {unit.order ?? 1}
          </span>
          <div className="min-w-0">
            <p className={`font-semibold truncate ${isUnlocked ? 'text-slate-900 dark:text-slate-100' : 'text-slate-500 dark:text-slate-400'}`}>
              {unit.title}
            </p>
            {unit.description && (
                          ...
                          <span
                            ...
                            onClick={() => {
                              if (lessonUnlocked) {
                                ...
                                navigate(`/lesson/${lesson._id}?subjectId=${subjectId}&from=student${assignmentParam}`);
                              }
                            }}
                          >
                            {lesson.title}
                      ...
                      <div className="flex items-center gap-2 ml-auto flex-shrink-0">
                        ...
                        {lessonUnlocked && (
                          <>
                            ...
                            <Button size="sm" variant="ghost" className="h-7 px-2.5 text-xs text-blue-600 dark:text-blue-400 hover:bg-blue-50 dark:hover:bg-blue-900/20"
                              onClick={() => {
                                const assignmentParam = assignmentId ? `&assignmentId=${assignmentId}` : '';
                                navigate(`/lesson/${lesson._id}?subjectId=${subjectId}&from=student${assignmentParam}`);
                              }}
    ...
    return (
      <div className="mt-8">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
          {[1, 2, 3].map((i) => (
            <div key={i} className="border rounded-xl p-4 border-slate-200 dark:border-slate-800 animate-pulse">
...
export default function StudentSubjectDetail() {
  ...
  const params = useParams<{ id?: string; subjectId?: string; teacherId?: string }>();
  const subjectId = params.subjectId ?? params.id;
  const teacherId = params.teacherId;
  const { user, isLoading: profileLoading } = useAuth();
  const queryClient = useQueryClient();
  const { pushToast } = useToast();
  const gradeId = (location.state as { gradeId?: string } | null)?.gradeId ?? user?.gradeId ?? '';
  const stageId = (location.state as { stageId?: string } | null)?.stageId ?? user?.stageId ?? '';

  type PaymentMethod = 'Vodafone Cash' | 'InstaPay';
  const unitPaymentMethods: { value: PaymentMethod; label: string }[] = [
    { value: 'Vodafone Cash', label: t('vodafoneCash') },
    { value: 'InstaPay', label: t('instaPay') },
  ];

  const [paymentOpen, setPaymentOpen] = useState(false);
  const [selectedUnit, setSelectedUnit] = useState<AssignmentContentUnit | null>(null);
  ...
  const { data: content, isLoading: contentLoading } = useQuery<SubjectTeacherContent>({
  ...
  const unitPrices = (content?.units ?? []).map((unit) => Number(unit.price) || 0).filter((price) => price > 0);
  ...
  const { data: schedules = [], isLoading: schedulesLoading } = useQuery<TeacherSchedule[]>({
  ...
  const requestMutation = useMutation({
    ...
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['subject-teacher-content', subjectId, teacherId] });
    },
  });
  ...
  const submitSubscriptionRequest = async () => {
    ...
    const teacherId = assignmentTeacherId;
    const gradeId = assignmentGradeId;
    const subjectIdValue = resolvedSubjectId;

    if (!teacherId || !gradeId || !subjectIdValue) {
  ...
  const teacherName = typeof assignment?.teacherId === 'string' ? undefined : assignment?.teacherId?.name;
    ...
    <div className={spacing.pageContainer}>
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 mb-8">
          ...
          {subjectLoading || !subject ? (
            ...
            <div className="flex items-center gap-2">
              ...
              <div>
                ...
                <p className="text-xs text-slate-400 dark:text-slate-500">{t('subjectPriceLabel')}: {subjectPrice} {currencyLabel}</p>
                {teacherName && (
                  <p className="text-xs text-slate-400 dark:text-slate-500">{t('teacher')}: {teacherName}</p>
                )}
              </div>
            </div>
          )}
        ...
        {!hasSubjectAccess && assignmentTeacherId && assignmentGradeId && (
          <Button className="bg-blue-600 hover:bg-blue-700 text-white" onClick={openSubjectDialog}>
      ...
      {showContentSkeleton ? (
        ...
        <div className="flex items-center gap-6 mb-6 px-1">
          ...
          <div className="flex items-center gap-2 text-sm text-slate-500 dark:text-slate-400">
            ...
            <span><strong className="text-slate-700 dark:text-slate-300">{subjectPrice}</strong> {currencyLabel} {t('subjectPriceLabel')}</span>
          </div>
          {minUnitPrice > 0 && (
            <div className="flex items-center gap-2 text-sm text-slate-500 dark:text-slate-400">
              <CreditCard className="w-4 h-4" />
              <span>
                <strong className="text-slate-700 dark:text-slate-300">
                  {minUnitPrice === maxUnitPrice ? minUnitPrice : `${minUnitPrice} - ${maxUnitPrice}`}
                </strong> {t('unitPriceLabel')}
              </span>
            </div>
          )}
          <div className="flex items-center gap-2 text-sm text-emerald-600 dark:text-emerald-400">
            <CheckCircle2 className="w-4 h-4" />
            <span><strong>{subscribedUnits}</strong> {t('enrolledBadge')}</span>
          </div>
          {lockedUnits > 0 && (
            <div className="flex items-center gap-2 text-sm text-slate-400 dark:text-slate-500">
              <Lock className="w-4 h-4" />
              <span><strong>{lockedUnits}</strong> {t('availableToSubscribe')}</span>
            </div>
          )}
        </div>
      )}
      ...
      {!teacherId ? (
        ...
        <div className="space-y-4">
          {units.map((unit) => {
            ...
            const hasPreview = !isSubscribed && unitLessons.some((lesson) => lesson.isUnlocked);
          ...
          <div className="space-y-4">
            <div className="rounded-xl border border-slate-200 dark:border-slate-800 p-4 bg-slate-50 dark:bg-slate-900/60">
              ...
              <p className="text-base font-semibold text-slate-900 dark:text-slate-100">
                {requestType === 'subject'
                  ? `${t('subject')}: ${getLocalizedName(subject, i18n.language)}`
                  : selectedUnit
                  ? `${t('unitSingular')}: ${selectedUnit.title}`
                  : '-'}
              </p>
              <p className="text-sm text-slate-500">
                {t('subjectPriceLabel')}: {requestType === 'unit' ? (selectedUnit?.price ?? subjectPrice) : subjectPrice} {currencyLabel}
              </p>
            </div>
            ...
            <div className="flex items-center justify-end gap-2">
              ...
              <Button
                ...
                disabled={uploadMutation.isPending || requestMutation.isPending}
              >
                {requestMutation.isPending ? t('loadingEllipsis') : t('submitPayment')}
              </Button>
            </div>
          </div>
        </DialogContent>
