/**
 * CourseLessonForm – shared lesson create/edit page for course-based lessons.
 *
 * Used by both:
 ...
 *
 * The back-path is derived automatically from the URL (everything before /lessons).
 * All lesson logic (create, update, parts) is identical regardless of role — the
 * API-level RBAC enforces ownership.
 */
...
import {
  ...
  createLessonPart,
  deleteLessonPart,
  type LessonInput as SubjectLessonInput,
  type LessonPart as LessonPartResponse,
  type LessonPartQuizItem,
} from '@/api/subjectApi';
import { createLessonForCourse, type LessonInput } from '@/api/courseApi';
...
export default function CourseLessonForm() {
  ...
  // Derive back path from current URL (strip /lessons/... suffix)
  const backPath = location.pathname.replace(/\/lessons.*/, '');

  // ── Fetch existing lesson + parts (edit mode) ───────────────────
  ...
  const initialData: Partial<LessonFormData> | undefined = isEditMode
    ? lessonData
      ? {
          ...
          parts: partsData.map(
            (p: LessonPartResponse): LessonPart => ({
              id: p._id,
              title: p.title,
              content: p.content ?? '',
              media: {
  ...
  // ── Save mutation (create or update + rebuild parts) ───────────
  const saveMutation = useMutation({
    mutationFn: async (data: LessonFormData) => {
      const payload: LessonInput & SubjectLessonInput = {
        title: data.title,
      ...
      if (isEditMode && lessonId) {
        ...
        // Delete all existing parts then recreate
        const existing = await getPartsByLesson(lessonId);
        await Promise.all(
          existing.map((p: LessonPartResponse) => deleteLessonPart(p._id))
        );
      } else {
        savedLesson = await createLessonForCourse(courseId!, payload);
      }
          ...
          data.parts.map((p, i) =>
            createLessonPart(savedLesson._id, {
              ...
              media: {
                ...p.media,
                videoUrl:
                  normalizeYouTubeUrl(p.media?.videoUrl) || p.media?.videoUrl,
              },
              quiz: p.quiz.map((q) => ({
                question: q.question,
                options: q.options,
  ...
  return (
    <FormPageLayout
      title={isEditMode ? t('editLesson') : t('addLesson')}
      subtitle={isEditMode ? t('enterLessonTitle') : undefined}
      backTo={backPath}
      backLabel={t('back')}
    >
