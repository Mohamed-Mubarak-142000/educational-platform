  ...
  return (
    <div className="min-h-screen bg-slate-50 dark:bg-slate-950 text-slate-900 dark:text-slate-50 font-sans selection:bg-violet-200 flex flex-col">
      ...
      <main className="flex-1">
        ...
        <div className="absolute top-0 right-0 -translate-y-1/3 translate-x-1/3 w-[720px] h-[720px] bg-violet-400/20 dark:bg-violet-900/20 rounded-full blur-[120px] pointer-events-none" />
        <div className="absolute bottom-0 left-0 translate-y-1/3 -translate-x-1/3 w-[520px] h-[520px] bg-purple-400/20 dark:bg-purple-900/20 rounded-full blur-[110px] pointer-events-none" />
        ...
        <div className="max-w-6xl mx-auto px-6 relative z-10 grid grid-cols-1 lg:grid-cols-2 gap-12 items-center">
          ...
          >
            <div className="inline-flex items-center gap-2 px-4 py-2 rounded-full bg-purple-100 dark:bg-purple-900/40 text-purple-700 dark:text-purple-300 font-semibold text-sm">
            ...
            <div className="grid gap-3 text-slate-600 dark:text-slate-300">
              {[t('featStudent2'), t('featStudent3'), t('featStudent4')].map((item) => (
                <div key={item} className={`flex items-center gap-3 ${isRtl ? 'flex-row-reverse justify-end' : ''}`}>
                  <div className="w-9 h-9 rounded-full bg-purple-100 dark:bg-purple-900/40 text-purple-600 dark:text-purple-300 flex items-center justify-center">
            ...
            <Card className="border border-slate-200 dark:border-slate-800 bg-white/80 dark:bg-slate-900/70 backdrop-blur-sm shadow-2xl rounded-3xl">
              ...
              <CardContent>
                <form onSubmit={registerForm.handleSubmit(onRegisterSubmit)} className="space-y-4">
                  ...
                  <div>
                    <select
                      ...
                      className="w-full rounded-md border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-900 px-3 py-2 text-sm text-slate-900 dark:text-slate-100 focus:outline-none focus:ring-2 focus:ring-purple-500"
                    >
                  ...
                  <div className="border border-slate-200 dark:border-slate-700 rounded-xl p-4 bg-violet-50/40 dark:bg-violet-900/10">
                    <label className="flex items-start gap-3 cursor-pointer">
                      ...
                      <div>
                        <p className="text-sm font-medium text-slate-800 dark:text-slate-200 flex items-center gap-1.5">
                          <Video className="w-4 h-4 text-violet-600" /> {t('subscribeLiveLessonsLabel')}
                        </p>
                  ...
                  <Button type="submit" className="w-full bg-purple-600 hover:bg-purple-700 text-white shadow-lg shadow-purple-600/30" disabled={registerMutation.isPending}>
                ...
                <div className="text-center mt-4 text-sm text-slate-500 dark:text-slate-400">
                  {t('authHaveAccount')} <Link to="/login" className="text-violet-600 dark:text-violet-400 hover:underline">{t('login')}</Link>
                </div>
              </CardContent>
            </Card>
          </motion.div>
